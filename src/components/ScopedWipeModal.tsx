...
export default function ScopedWipeModal({ scope, onClose }: Props) {
  ...
  const preview = useMemo(
    () => buildWipePlan(scope, { units, aggregates, sales, accessoryStock, accessoryStockEvents }),
    [scope, units, aggregates, sales, accessoryStock, accessoryStockEvents],
  );
  ...
  const handleWipe = async () => {
    ...
    try {
      ...
      const plan = buildWipePlan(scope, { units, aggregates, sales, accessoryStock, accessoryStockEvents, events });
      if (plan.total === 0) {
